...
app.get("/login", (req,res)=> {
    ...
    if(req.query.logOutSuccessfully)data.logOutSuccessfully = true;
    res.render("login", data);
});
app.get("/logout", (req,res)=>{
    let data = {};
    req.session.destroy();
    res.redirect("/login?loggedOutSuccessfully");
});
...
app.post("/login", async (req,res)=>{
    try{
        ...
        console.log(dbUser);
        if(!dbUser) throw new Error("Login Fails");
        bcrypt.compare(req.body.password, dbUser.password, (err, same)=>{
            if (err) throw err;
            // check the password matches
            if (!same) throw new Error("Incorrect information");
